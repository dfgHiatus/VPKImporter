...
internal class VPKExtractor
{
    private static readonly string executablePath = Path.Combine("rml_mods", "vpk_extractor");
    ...
    internal static void Unpack(string inputFile, string outputPath, byte optionalThreads)
    {
        ...
        switch (platform)
        {
            ...
            default:
                throw new PlatformNotSupportedException("You've managed to run this on a platform .NET doesn't recognize, how are you playing Resonite?");
        }
    }
    ...
    private static void PerformWindowsUnpack (string inputFile, string outputPath, byte optionalThreads)
    {
        ...
        if (!File.Exists(windowsExecutablePath))
            throw new FileNotFoundException("Could not find ValveResourceFormat decompiler. Is it present under rml_mods/vpk_extractor/Decompiler.exe?");
